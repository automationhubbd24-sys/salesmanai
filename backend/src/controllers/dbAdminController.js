...
exports.updateRow = async (req, res) => {
    try {
        ...
        // --- SPECIAL LOGIC: Auto-Unlock for api_list ---
        if (table === 'api_list' && row.status === 'active') {
            setFragments.push(`cooldown_until = NULL`);
            setFragments.push(`usage_today = 0`);
            setFragments.push(`usage_tokens_today = 0`);
            console.log(`[DBAdmin] Manually unlocking API Key via status change to active.`);
        }
        ...
        // --- NEW: Sync with KeyService Cache immediately after manual unlock ---
        if (table === 'api_list') {
            const keyService = require('../services/keyService');
            if (keyService.updateKeyCache) {
                await keyService.updateKeyCache(true);
                console.log(`[DBAdmin] API Key Cache synchronized after manual update.`);
            }
        }
...
exports.updateEngineOverride = async (req, res) => {
    try {
        const { platform, id, engine_override } = req.body;
        ...
        await pgClient.query(sql, [engine_override || null, id]);

        res.json({ success: true });
    } catch (error) {
        console.error('DB Admin updateEngineOverride error:', error);
        res.status(500).json({ success: false, error: error.message });
    }
};
...
exports.getSemanticCacheConfigs = async (req, res) => {
    try {
        ...
        try {
            ...
            await pgClient.query(`ALTER TABLE whatsapp_message_database ADD COLUMN IF NOT EXISTS engine_override VARCHAR(50)`);
            await pgClient.query(`ALTER TABLE whatsapp_message_database ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW()`);

            // Page Table Repair
            await pgClient.query(`ALTER TABLE page_access_token_message ADD COLUMN IF NOT EXISTS engine_override VARCHAR(50)`);
