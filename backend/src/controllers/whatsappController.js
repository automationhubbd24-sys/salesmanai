...
function buildResolvedProductContext(product, queryText = '', preferredSkuKey = null) {
    const resolved = dbService.resolveProductSkuSelection(product, queryText, preferredSkuKey);
    const baseProduct = resolved.product || product;
    const selectedSku = resolved.selectedSku;
    const mediaImages = [];
    const mediaVideos = [];

    if (baseProduct.image_url) mediaImages.push(baseProduct.image_url);
    if (baseProduct.video_url) mediaVideos.push(baseProduct.video_url);

    // Add additional images from base product
    let additional = [];
    if (Array.isArray(baseProduct.additional_images)) {
        additional = baseProduct.additional_images;
    } else if (typeof baseProduct.additional_images === 'string' && baseProduct.additional_images.trim()) {
        try {
            additional = JSON.parse(baseProduct.additional_images);
        } catch (e) {
            additional = baseProduct.additional_images.split(',').map(s => s.trim());
        }
    }
    ...
    if (selectedSku?.image_url) mediaImages.push(selectedSku.image_url);
    if (selectedSku?.video_url) mediaVideos.push(selectedSku.video_url);
...
async function processBufferedMessages(sessionId, sessionName, senderId, messages, pageId = null, preLoadedConfig = null, lockSenderId = null) {
    ...
    // Reset combinedText to re-process with reply context if needed below, 
    // or just keep it and add reply context. Let's reset for consistency with existing logic.
    combinedText = ""; 
                    ...
                    if (product) {
                        ...
                        urls.forEach(u => {
                            if (!extractedImages.some(img => img.url === u)) {
                                extractedImages.push({ url: u, title: product.name });
                            }
                        });
                    }
                }
