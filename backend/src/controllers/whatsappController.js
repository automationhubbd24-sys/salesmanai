        ...
        dbService.getConversationState(sessionName, senderId).then(async (state) => {
            ...
            if (cached) {
                ...
                if (pageConfig && (pageConfig.semantic_cache_enabled === true || pageConfig.semantic_cache_enabled === 1)) {
                    ...
                    const hasAdminReplied = await dbService.hasWhatsAppAdminReplySince(sessionName, senderId, triggerTimestamp);
...
এটি মূলত একটি **"[কম্বো বা অফার নাম]"** হিসেবে সাজানো হয়েছে। [একটি ছোট বাক্যে সারসংক্ষেপ]`;
    ...
    try {
        ...
        */
        ...
        // Check if admin replied
        const hasAdminReplied = await dbService.hasWhatsAppAdminReplySince(sessionName, effectiveSenderId, triggerTimestamp);
