...
এটি মূলত একটি **"[কম্বো বা অফার নাম]"** হিসেবে সাজানো হয়েছে। [একটি ছোট বাক্যে সারসংক্ষেপ]`;
            ...
            for (let idx = 0; idx < allAnalysisResults.length; idx++) {
                ...
                if (perMsgText) {
                    ...
                    try {
                        // Run Vector Search using the analysis text
                        const searchResults = await aiService.searchProductByImageVector(perMsgText, pageId);
                        if (searchResults && searchResults.length > 0) {
                            const topMatches = searchResults.slice(0, 3).map(p => `ID: ${p.product_id} | Name: ${p.name} | Score: ${p.match_score} | DB Image: ${p.image_url}`).join('\n');
                            combinedImageAnalysis += `[VECTOR SEARCH SYSTEM RESULT - POSSIBLE MATCHES FROM OUR CATALOG]:\n${topMatches}\n\n`;
                        } else {
                            combinedImageAnalysis += `[VECTOR SEARCH SYSTEM RESULT]: No exact matching design found in catalog for this image.\n\n`;
                        }
                    } catch (searchErr) {
                        console.warn("[Messenger] Vector Search failed:", searchErr.message);
                    }
