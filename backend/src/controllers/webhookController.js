...
async function processBufferedMessages(sessionId, pageId, senderId, messages) {
    ...
    console.log(`Processing buffered messages for ${sessionId}. Text: ${combinedText.substring(0,50)}... Images: ${allImages.length}, Audios: ${allAudios.length}`);
        ...
        if (isBanned || (!hasAnyCredit && !hasOwnKey)) {
             const logMsg = `Page ${pageConfig.page_id} blocked. Status: ${pageConfig.subscription_status}, Credits: B:${pageConfig.bonus_credit} P:${pageConfig.permanent_credit} D:${pageConfig.daily_limit} M:${pageConfig.message_credit}, OwnKey: ${hasOwnKey}`;
             console.log(logMsg);
             logToFile(logMsg);
