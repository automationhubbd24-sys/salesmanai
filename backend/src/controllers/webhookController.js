...
// #region debug-point whatsapp-product-image-reporter
function reportWhatsAppProductImageDebug(hypothesisId, location, msg, data = {}) {
    try {
        const envPath = path.resolve(__dirname, '../../../.dbg/whatsapp-product-image.env');
        let debugUrl = 'http://127.0.0.1:7777/event';
        let sessionId = 'whatsapp-product-image';
        try {
            const envContent = fs.readFileSync(envPath, 'utf8');
            debugUrl = envContent.match(/^DEBUG_SERVER_URL=(.+)$/m)?.[1]?.trim() || debugUrl;
            sessionId = envContent.match(/^DEBUG_SESSION_ID=(.+)$/m)?.[1]?.trim() || sessionId;
        } catch (_) {}
        fetch(debugUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function analyzeAndMatchIncomingImage({ platform, pageId, senderId, imageUrl, imageIndex, batchId, pageConfig, prompt }) {
    return incomingImageAnalysisService.analyzeAndMatchIncomingImage({
        platform,
        pageId,
        senderId,
        imageUrl,
...
async function processWhatsAppBatch(bufferedMessages, config, pagePrompts, senderName, senderId, wabaId, phoneNumberId, identity = {}) {
    ...
    if (allImages.length > 0) {
        ...
        if (!imageDetectionEnabled) {
            ...
            const productAnalysisPrompt = incomingImageAnalysisService.resolveIncomingImagePrompt({
                pagePrompts: controlConfig,
                pageConfig: config,
                fallbackPrompt: 'Analyze this image with 100% precision. Focus on products and text. Output in Bengali format.'
            });
    ...
    // #region debug-point whatsapp-product-image-A
    reportWhatsAppProductImageDebug('A', 'webhookController.js:1964', 'AI structured delivery decision', {
        action: aiResponse?.action || null,
        productId: aiResponse?.product_id || null,
        items: structuredDeliveryItems.map((item) => ({ action: item.action || null, productId: item.product_id || null })),
        hasProductTag: /\[PRODUCT_ID\s*:/i.test(finalReplyText || '')
    });
    // #endregion
    ...
    // #region debug-point whatsapp-product-image-B
    reportWhatsAppProductImageDebug('B', 'webhookController.js:2130', 'Media state after product tag and photo fallback', {
        imageCount: Array.isArray(aiResponse?.images) ? aiResponse.images.length : 0,
        videoCount: Array.isArray(aiResponse?.videos) ? aiResponse.videos.length : 0,
        hasPhotoIntent: hasPhotoIntent(recentRawHistory),
    ...
    // #region debug-point whatsapp-product-image-C
    reportWhatsAppProductImageDebug('C', 'webhookController.js:2288', 'WhatsApp delivery queue and image gate decision', {
        imageSendEnabled: allowImageSend,
        deliveryQueueCount: deliveryQueue.length,
        sendQueueCount: sendQueue.length,
        entries: sendQueue.map((entry) => ({ imageCount: entry.images.length, videoCount: entry.videos.length, textCount: entry.text ? 1 : 0 }))
    });
    // #endregion
    ...
    for (let queueIndex = 0; queueIndex < sendQueue.length; queueIndex++) {
        ...
        for (let i = 0; i < entry.images.length; i++) {
            ...
            // #region debug-point whatsapp-product-image-D
            let imageUrlHostname = null;
            try { imageUrlHostname = new URL(image.url).hostname; } catch (_) {}
            reportWhatsAppProductImageDebug('D', 'webhookController.js:2373', 'Before WhatsApp image API call', {
                urlHostname: imageUrlHostname,
                hasCaption: Boolean(caption)
            });
            // #endregion
            let imageSendResponse;
            try {
                imageSendResponse = await whatsappCloudService.sendImageMessage(resolvedPhoneNumberId, config.cloud_access_token, replyRecipientId, image.url, caption);
            } catch (error) {
                // #region debug-point whatsapp-product-image-F
                reportWhatsAppProductImageDebug('F', 'webhookController.js:2384', 'WhatsApp image send failed', {
                    httpStatus: error?.response?.status || null,
                    providerCode: error?.response?.data?.error?.code || null,
                    message: String(error?.response?.data?.error?.message || error?.message || '').slice(0, 200)
                });
                // #endregion
                throw error;
            }
            // #region debug-point whatsapp-product-image-E
            reportWhatsAppProductImageDebug('E', 'webhookController.js:2392', 'WhatsApp image send succeeded', {
                hasProviderMessageId: Boolean(imageSendResponse?.messages?.[0]?.id)
            });
            // #endregion
        }
...
এটি মূলত একটি **"[কম্বো বা অফার নাম]"** হিসেবে সাজানো হয়েছে। [একটি ছোট বাক্যে সারসংক্ষেপ]`;
                ...
                productAnalysisPrompt = incomingImageAnalysisService.resolveIncomingImagePrompt({
                    pagePrompts,
                    pageConfig,
                    fallbackPrompt: productAnalysisPrompt
                });
    ...
    console.log(`[Context] Using last ${effectiveHistory.length} messages (Limit: ${historyLimit})`);
        ...
        reportWhatsAppProductImageDebug('A', 'webhookController.js:3993', 'Messenger AI structured delivery decision', {
            ...
            productId: aiResponse?.product_id || null,
            items: structuredDeliveryItems.map((item) => ({ action: item.action || null, productId: item.product_id || null })),
            hasProductTag: /\[PRODUCT_ID\s*:/i.test(replyText || '')
        });
        // #endregion
        ...
        // #region debug-point whatsapp-product-image-messenger-B
        reportWhatsAppProductImageDebug('B', 'webhookController.js:4296', 'Messenger media state after product tag and photo fallback', {
            platform: 'messenger',
            imageCount: Array.isArray(aiResponse?.images) ? aiResponse.images.length : 0,
            videoCount: Array.isArray(aiResponse?.videos) ? aiResponse.videos.length : 0,
            hasPhotoIntent: hasPhotoIntent(effectiveHistory),
            productId: effectiveProductId
        });
        // #endregion
        ...
        // #region debug-point whatsapp-product-image-messenger-C
        const messengerImageSendEnabled = !pagePrompts || (pagePrompts.image_send !== false && pagePrompts.image_send !== 'false' && pagePrompts.image_send !== 0 && pagePrompts.image_send !== '0');
        reportWhatsAppProductImageDebug('C', 'webhookController.js:4940', 'Messenger final outgoing media queue and image gate decision', {
            platform: 'messenger',
            imageSendEnabled: messengerImageSendEnabled,
            imageSendConfig: pagePrompts?.image_send ?? null,
            imageCount: Array.isArray(aiResponse?.images) ? aiResponse.images.length : 0,
            videoCount: Array.isArray(aiResponse?.videos) ? aiResponse.videos.length : 0
        });
        // #endregion
            ...
            if (!allowImageSend) {
                ...
                if (useCarousel && images.length > 1) {
                    ...
                    try {
                        ...
                        // #region debug-point whatsapp-product-image-messenger-D
                        const carouselUrlHostnames = carouselElements.map((element) => {
                            try { return new URL(element.image_url).hostname; } catch (_) { return null; }
                        });
                        reportWhatsAppProductImageDebug('D', 'webhookController.js:5007', 'Before Messenger carousel image API call', {
                            platform: 'messenger',
                            urlHostnames: carouselUrlHostnames
                        });
                        // #endregion
                        let carouselResult;
                        try {
                            carouselResult = await facebookService.sendCarouselMessage(pageId, senderId, carouselElements, pageConfig.page_access_token);
                        } catch (carouselError) {
                            // #region debug-point whatsapp-product-image-messenger-F
                            reportWhatsAppProductImageDebug('F', 'webhookController.js:5017', 'Messenger carousel image send failed', {
                                platform: 'messenger',
                                httpStatus: carouselError?.response?.status || null,
                                providerCode: carouselError?.response?.data?.error?.code || null,
                                message: String(carouselError?.response?.data?.error?.message || carouselError?.message || '').slice(0, 200)
                            });
                            // #endregion
                            throw carouselError;
                        }
                        // #region debug-point whatsapp-product-image-messenger-E
                        reportWhatsAppProductImageDebug('E', 'webhookController.js:5027', 'Messenger carousel image send succeeded', {
                            platform: 'messenger',
                            hasProviderMessageId: Boolean(carouselResult?.message_id)
                        });
                        // #endregion
                ...
                if (!sentViaCarousel) {
                    ...
                    for (const imgObj of images) {
                         try {
                             ...
                             // #region debug-point whatsapp-product-image-messenger-D
                             let imageUrlHostname = null;
                             try { imageUrlHostname = new URL(imgObj.url).hostname; } catch (_) {}
                             reportWhatsAppProductImageDebug('D', 'webhookController.js:5068', 'Before Messenger image upload API call', {
                                 platform: 'messenger',
                                 urlHostname: imageUrlHostname
                             });
                             // #endregion
                             let uploadResult;
                             try {
                                 uploadResult = await facebookService.sendImageUpload(pageId, senderId, imgObj.url, pageConfig.page_access_token);
                             } catch (imgError) {
                                 // #region debug-point whatsapp-product-image-messenger-F
                                 reportWhatsAppProductImageDebug('F', 'webhookController.js:5078', 'Messenger image upload failed', {
                                     platform: 'messenger',
                                     httpStatus: imgError?.response?.status || null,
                                     providerCode: imgError?.response?.data?.error?.code || null,
                                     message: String(imgError?.response?.data?.error?.message || imgError?.message || '').slice(0, 200)
                                 });
                                 // #endregion
                                 throw imgError;
                             }
                             // #region debug-point whatsapp-product-image-messenger-E
                             reportWhatsAppProductImageDebug('E', 'webhookController.js:5088', 'Messenger image upload succeeded', {
                                 platform: 'messenger',
                                 hasProviderMessageId: Boolean(uploadResult?.message_id)
                             });
                             // #endregion
                             ...
                             try {
                                 ...
                                 // #region debug-point whatsapp-product-image-messenger-D
                                 let fallbackUrlHostname = null;
                                 try { fallbackUrlHostname = new URL(imgObj.url).hostname; } catch (_) {}
                                 reportWhatsAppProductImageDebug('D', 'webhookController.js:5116', 'Before Messenger direct image API call', {
                                     platform: 'messenger',
                                     urlHostname: fallbackUrlHostname
                                 });
                                 // #endregion
                                 let directImageResult;
                                 try {
                                     directImageResult = await facebookService.sendImageMessage(pageId, senderId, imgObj.url, pageConfig.page_access_token);
                                 } catch (urlError) {
                                     // #region debug-point whatsapp-product-image-messenger-F
                                     reportWhatsAppProductImageDebug('F', 'webhookController.js:5126', 'Messenger direct image send failed', {
                                         platform: 'messenger',
                                         httpStatus: urlError?.response?.status || null,
                                         providerCode: urlError?.response?.data?.error?.code || null,
                                         message: String(urlError?.response?.data?.error?.message || urlError?.message || '').slice(0, 200)
                                     });
                                     // #endregion
                                     throw urlError;
                                 }
                                 // #region debug-point whatsapp-product-image-messenger-E
                                 reportWhatsAppProductImageDebug('E', 'webhookController.js:5136', 'Messenger direct image send succeeded', {
                                     platform: 'messenger',
                                     hasProviderMessageId: Boolean(directImageResult?.message_id)
                                 });
                                 // #endregion
                             } catch (urlError) {
