                            ...
                            if (value.messages) {
                                for (const message of value.messages) {
                                    ...
                                    if (messageType === 'image' && message.image) {
                                        try {
                                            ...
                                            const mediaResponse = await axios.get(mediaUrl, {
                                                headers: { 'Authorization': `Bearer ${config.cloud_access_token}` }
                                            });
                                            if (mediaResponse.data.url) {
                                                imageUrls.push(mediaResponse.data.url);
                                                console.log(`[WhatsApp Cloud] Image received: ${mediaResponse.data.url}`);
                                            }
                                        } catch (e) {
                                            console.warn(`[WhatsApp Cloud] Failed to fetch image URL: ${e.message}`);
                                        }
                                        ...
                                        try {
                                            ...
                                            if (mediaResponse.data.url) {
                                                audioUrls.push(mediaResponse.data.url);
                                                console.log(`[WhatsApp Cloud] Audio received: ${mediaResponse.data.url}`);
                                            }
                                        } catch (e) {
                                            console.warn(`[WhatsApp Cloud] Failed to fetch audio URL: ${e.message}`);
                                        }
                                    }

                                    if (!messageText && imageUrls.length === 0 && audioUrls.length === 0) continue;

                                    console.log(`[WhatsApp Cloud] Message from ${senderName} (${senderId}): ${messageText || '[Media Message]'}`);
                                    ...
                                    const aiResponse = await aiService.generateResponse({
                                        pageId: wabaId,
                                        userId: senderId,
                                        userMessage: messageText || (imageUrls.length > 0 ? '[User sent images]' : (audioUrls.length > 0 ? '[User sent voice message]' : '')),
