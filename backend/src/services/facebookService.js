...
async function reactToComment(commentId, reactionType, accessToken) {
    const type = String(reactionType || '').trim().toUpperCase();
    if (!['LIKE', 'LOVE', 'WOW', 'HAHA', 'SAD', 'ANGRY'].includes(type)) {
        throw new Error(`Unsupported Facebook comment reaction: ${reactionType}`);
    }

    try {
        const url = `https://graph.facebook.com/${FACEBOOK_GRAPH_VERSION}/${commentId}/reactions?access_token=${accessToken}`;
        console.log(`Reacting to comment ${commentId} with ${type}`);
