    ...
    try {
        switch (name) {
            ...
            case 'check_stock': {
                ...
                let stock = product.stock_quantity !== undefined ? product.stock_quantity : (product.stock !== undefined ? product.stock : 'Unknown');
                if (args.sku_code || args.variant_key || args.option_text) {
                    ...
                    if (resolved.selectedSku) {
                        stock = resolved.selectedSku.stock;
                    }
                }
        ...
        try {
            ...
            candidates.slice(0, 5).forEach((p, idx) => {
                ...
                if (resolvedContext.selectedSku) {
                    ...
                    snapshot += `   Exact SKU Stock: ${selectedSku.stock ?? p.stock ?? 'Unknown'}\n`;
                ...
                if (Array.isArray(p.sku_matrix) && p.sku_matrix.length > 0) {
                    ...
                    p.sku_matrix.forEach((sku) => {
                        ...
                        snapshot += `     - ${sName} (${sku.sku_code}) | Price: ${sPrice} | Stock: ${sku.stock ?? p.stock ?? 'Unknown'}\n`;
                    });
