...
async function resolveSalesmanchatbotEngine(pageConfig, defaultProvider, defaultModel, isVision, isAudio, isEmbedding = false) {
    ...
    if (isEmbedding) {
        ...
        finalModel = brandedConfig.embed_model || 'google/gemini-embedding-001';
        modality = 'embedding';
    } else if (isAudio) {
...
async function getEmbedding(text, customApiKey = null) {
    ...
    try {
        ...
        const vector = normalizeEmbeddingVector(response.data.data[0].embedding, internalModel || 'google/gemini-embedding-001');
        if (vector) {
        ...
        try {
            ...
            if (provider === 'google' || provider === 'gemini') {
                ...
                const modelName = (config && config.model) || "google/gemini-embedding-001";
                const model = genAI.getGenerativeModel({ model: modelName });
                const result = await model.embedContent(text.replace(/\n/g, ' '));
                vector = normalizeEmbeddingVector(result.embedding.values, modelName);
            } else {
                const modelName = (config && config.model) || 'text-embedding-3-small';
                ...
                vector = normalizeEmbeddingVector(res.data[0].embedding, modelName);
            }
        ...
        switch (name) {
            case 'resolve_product': {
                ...
                const candidates = products.map(p => {
                    ...
                    const distance = typeof p.distance === 'number' && !isNaN(p.distance) ? p.distance : 0.5;
...
async function generateReply(userMessage, pageConfig, pagePrompts, history = [], senderName = 'Customer', ownerName = 'Automation Hub BD', senderGender = null, imageUrls = [], audioUrls = [], extraTokenUsage = 0, userId = null, pageId = null) {
    ...
    const buildPromptProductSnapshot = async (queryText) => {
        ...
        try {
            const candidates = await dbService.searchProductsForResource(queryText, pageConfig.page_id);
            if (!candidates || candidates.length === 0) return "";

            let snapshot = "[PRODUCT LIST SNAPSHOT - FROM PRODUCT ENTRY]\n";
            candidates.slice(0, 5).forEach((p, idx) => {
                ...
                const resolvedContext = dbService.resolveProductSkuSelection(p, queryText);
                ...
                if (Array.isArray(p.sku_matrix) && p.sku_matrix.length > 0) {
                    ...
                    p.sku_matrix.forEach((sku) => {
                        ...
                        if (sku.image_url) {
                            skuLine += ` | Image: ${normalizeProductUrl(sku.image_url)}`;
                        }
                        snapshot += `${skuLine}\n`;
                    });
                ...
                if (p.is_combo && Array.isArray(p.combo_items) && p.combo_items.length > 0) {
                    snapshot += `   Combo Items: ${p.combo_items.join(', ')}\n`;
                }

                if (p.allow_description !== false && p.description) {
                    snapshot += `   Description: ${p.description}\n`;
                    if (p.is_combo) {
                        snapshot += `   Note: This is a combo. Check the description and combo items when the customer asks about included sub-items.\n`;
                    }
                }
                ...
                if (Array.isArray(p.additional_images) && p.additional_images.length > 0) {
                    snapshot += `   More Images: ${p.additional_images.map(normalizeProductUrl).join(', ')}\n`;
                }
            });
            snapshot += "\n";
            // #region debug-point D:product-snapshot
            (()=>{const _p='.dbg/variant-primary-image.env';let _u='http://127.0.0.1:7777/event',_s='variant-primary-image';try{const _e=fs.readFileSync(_p,'utf8');_u=_e.match(/DEBUG_SERVER_URL=(.+)/)?.[1]||_u;_s=_e.match(/DEBUG_SESSION_ID=(.+)/)?.[1]||_s}catch{}fetch(_u,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sessionId:_s,runId:'pre-fix',hypothesisId:'D',location:'aiService.js:buildPromptProductSnapshot',msg:'[DEBUG] product snapshot candidate resolution',data:{queryText,candidates:candidates.slice(0,5).map((p)=>{const r=dbService.resolveProductSkuSelection(p,queryText);return{id:p.id,name:p.name,image_url:p.image_url,selectedSkuKey:r.selectedSku?.key||null,selectedSkuName:r.selectedSku?.name||null,selectedSkuImage:r.selectedSku?.image_url||null,missingAttributes:Array.isArray(r.missingAttributes)?r.missingAttributes.map((item)=>item?.label||item?.name||null):[]};})},ts:Date.now()})}).catch(()=>{})})();
            // #endregion
            console.log(`[AI] Injected ${Math.min(candidates.length, 5)} product snapshot items for query.`);
            return snapshot;
        } catch (err) {
