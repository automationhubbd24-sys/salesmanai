...
async function resolveSalesmanchatbotEngine(pageConfig, defaultProvider, defaultModel, isVision, isAudio) {
    ...
    // 1. Fetch Branded Config FIRST (User's primary mapping from 1st screenshot)
    const brandedConfig = await getBrandedEngineConfig(targetEngineName);
    
    if (brandedConfig) {
        // Use mapping from branded config (engine_configs table)
        targetProvider = brandedConfig.provider || targetProvider;
        
        // If it's a known branded engine name but targetProvider is not set, apply hardcoded defaults
        if (!brandedConfig.provider) {
            if (targetEngineName === 'salesmanchatbot-pro') targetProvider = 'google';
            else if (targetEngineName === 'salesmanchatbot-flash') targetProvider = 'openrouter';
            else if (targetEngineName === 'salesmanchatbot-lite') targetProvider = 'groq';
        }
    } else {
    ...
    // 2. Fetch Global Config (Rate Limits and Provider-wide defaults from 2nd screenshot)
    ...
    // 3. APPLY BRANDED CONFIG OVERRIDES (Highest Priority - 1st Screenshot)
    if (brandedConfig) {
        engineTextModel = brandedConfig.text_model || engineTextModel;
        engineVisionModel = brandedConfig.image_model || engineVisionModel;
        engineVoiceModel = brandedConfig.voice_model || engineVoiceModel;
