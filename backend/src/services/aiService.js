...
const commandApiService = require('./commandApiService');
...
const proPlusResolver = require('./proPlusResolver');
const proPlusMediaChain = require('./proPlusMediaChain');
const proPlusEmbeddingChain = require('./proPlusEmbeddingChain');
...
async function getEmbedding(text, customApiKey = null, pageConfig = null) {
    if (!text) return null;
    ...
    // --- PRO_PLUS_TEST: Use salesmanchatbot-pro-plus embedding chain if enabled ---
    if (pageConfig && (pageConfig.pro_plus_test === true || pageConfig.pro_plus_test === 'true')) {
        console.log(`[AI Embedding] ProPlus Test Mode - using pro-plus embedding chain`);
        try {
            const embedResult = await proPlusEmbeddingChain.generateProPlusEmbedding(text, pageConfig);
...
async function generateReply(userMessage, pageConfig, pagePrompts, history = [], senderName = 'Customer', ownerName = 'Automation Hub BD', senderGender = null, imageUrls = [], audioUrls = [], extraTokenUsage = 0, userId = null, pageId = null) {
    ...
    const finalize = async (result) => {
        ...
        try {
            ...
            if (autosaveEnabled && (semEnabled || embedEnabled) && !usedSemanticCache && canCache && result && result.reply && cleanUserMessage) {
                if (embedEnabled) {
                    getEmbedding(cleanUserMessage, null, pageConfig).then(v => {
    ...
    try {
        ...
        if (semEnabled && !isMediaTurn && cleanUserMessage) {
            ...
            if (embedEnabled) {
                userMessageVector = await getEmbedding(cacheQuery, null, pageConfig);
            }
...
${productContext}`;
    ...
    */
    ...
    const isBrandedModel = ['salesmanchatbot-pro', 'salesmanchatbot-flash', 'salesmanchatbot-lite', 'salesmanchatbot-pro-plus'].includes(userModel);
...
`;
    ...
    if (hasOwnKey && !isOurOwnProvider) {
        ...
        for (const currentKey of userKeys) {
            ...
            try {
                ...
                const isBranded = ['salesmanchatbot-pro', 'salesmanchatbot-flash', 'salesmanchatbot-lite', 'salesmanchatbot-pro-plus'].includes(modelToUse);
    ...
    // --- PRO_PLUS_TEST: Route to salesmanchatbot-pro-plus chain if enabled ---
    if (pageConfig.pro_plus_test === true || pageConfig.pro_plus_test === 'true') {
        console.log(`[AI] ProPlus Test Mode ACTIVE for page ${pageConfig.page_id} (Modality: ${isVision ? 'vision' : (isAudio ? 'audio' : 'text')})`);

        try {
            if (isAudio) {
                const audioResult = await proPlusMediaChain.processProPlusAudio(audioUrls[0], pageConfig);
                if (audioResult.text) {
                    return finalize({
                ...
                if (textResult.reply) {
                    const structuredText = extractJsonFromAiResponse(textResult.reply);
                    return finalize({
                        reply: structuredText.reply || textResult.reply,
                        action: structuredText.action || "NONE",
                        product_id: structuredText.product_id || null,
    ...
    for (const currentModel of modelsToTry) {
        ...
        while (modelRetryCount < MAX_RETRIES_PER_MODEL) {
            ...
            try {
                ...
                const isBrandedEngine = ['salesmanchatbot-pro', 'salesmanchatbot-flash', 'salesmanchatbot-lite', 'salesmanchatbot-pro-plus'].includes(resolved.targetEngineName);
...
- Single line output only.`;
    ...
    let resolved = null;
    if (providerHint === 'salesmanchatbot' || modelHint === 'salesmanchatbot-pro' || modelHint === 'salesmanchatbot-flash' || modelHint === 'salesmanchatbot-lite' || modelHint === 'salesmanchatbot-pro-plus') {
    ...
    for (const currentModel of modelsToTry) {
        ...
        while (modelRetryCount < MAX_RETRIES_PER_MODEL) {
            ...
            try {
                ...
                // 2. Setup Proxy
                const isBranded = ['salesmanchatbot-pro', 'salesmanchatbot-flash', 'salesmanchatbot-lite', 'salesmanchatbot-pro-plus'].includes(resolved.targetEngineName || modelHint);
                let proxyAgent = null;
                if (isBranded) {
...
async function transcribeAudio(audioUrl, config) {
    ...
    let resolved = null;
    if ((providerHint === 'salesmanchatbot' || modelHint === 'salesmanchatbot-pro' || modelHint === 'salesmanchatbot-flash' || modelHint === 'salesmanchatbot-lite' || modelHint === 'salesmanchatbot-pro-plus') && !safeConfig.api_key) {
    ...
    if (!userKey && !isOwnAPI) {
        ...
        if (voiceModel === 'salesmanchatbot-pro') {
            ...
            provider = 'groq';
        } else if (voiceModel === 'salesmanchatbot-pro-plus') {
            voiceModel = 'gemini-1.5-flash';
            provider = 'google';
        }
    ...
    for (const option of priorityChain) {
        ...
        while (modelRetryCount < MAX_RETRIES_PER_MODEL) {
            ...
            try {
                ...
                // 2. Setup Proxy
                const isBrandedEngine = ['salesmanchatbot-pro', 'salesmanchatbot-flash', 'salesmanchatbot-lite', 'salesmanchatbot-pro-plus'].includes(resolved?.targetEngineName || modelHint || config.chat_model);
