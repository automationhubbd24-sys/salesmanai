...
function isKeyWithinLimits(keyData, requestedModel = null) {
    ...
    let tpmLimit = parseInt(keyData.tpm_limit);
    if (!(tpmLimit > 0) && manual && manual.tpm) {
    ...
    // TPD (Tokens Per Day)
    let tpdLimit = parseInt(keyData.tpd_limit);
    if (!(tpdLimit > 0) && manual && manual.tpd) {
    ...
    let tpmoLimit = parseInt(keyData.tpmo_limit);
    if (!(tpmoLimit > 0) && manual && manual.tpmo) {
...
async function getSmartKey(provider, model = 'default') {
    ...
    try {
        ...
        for (let i = 0; i < totalKeys; i++) {
            ...
            const tpmLimit = Math.min(
                parseInt(candidateKey.tpm_limit) || 99999,
                (globalLim && globalLim.tpm > 0) ? globalLim.tpm : 99999
            );
            const tpdLimit = Math.min(
                parseInt(candidateKey.tpd_limit) || 99999,
                (globalLim && globalLim.tpd > 0) ? globalLim.tpd : 99999
            );
            const tpmoLimit = Math.min(
                parseInt(candidateKey.tpmo_limit) || 99999,
                (globalLim && globalLim.tpmo > 0) ? globalLim.tpmo : 99999
            );
