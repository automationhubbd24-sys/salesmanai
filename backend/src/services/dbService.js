...
function buildAutoSkuMatrix(attributeSchema = [], productDefaults = {}) {
    ...
    return combinations.map((attributes, index) => ({
        ...
        stock: productDefaults.stock !== undefined && productDefaults.stock !== null && productDefaults.stock !== ''
            ? parseInt(productDefaults.stock, 10) || 0
            : 0,
...
function normalizeSkuMatrix(rawSkuMatrix, productDefaults = {}, attributeSchema = []) {
    ...
    const normalized = source.map((item, index) => {
        ...
        return {
            ...
            stock: item?.stock !== undefined && item?.stock !== null && item?.stock !== '' ? parseInt(item.stock, 10) || 0 : parseInt(productDefaults.stock, 10) || 0,
...
function buildLegacyVariantsFromProduct(product) {
    ...
    if (skuMatrix.length > 0) {
        return skuMatrix.map((sku, index) => ({
            ...
            available: sku.available !== false && Number(sku.stock ?? 0) !== 0,
