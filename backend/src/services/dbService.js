...
async function initTables() {
    try {
        ...
        await query(`
            ...
            ALTER TABLE whatsapp_message_database ADD COLUMN IF NOT EXISTS engine_override TEXT DEFAULT 'WEBJS';
        `);
...
async function createWhatsAppEntry(sessionName, userId, planDays = 30, initialStatus = 'connected', userEmail = null, engine = 'WEBJS', officialConfig = {}) {
    const { query } = require('./pgClient');
    ...
    const insertResult = await query(
        `INSERT INTO whatsapp_message_database
            (session_name, user_id, email, active, status, reply_message, order_tracking, subscription_status, text_prompt, expires_at, plan_days, ai_provider, chat_model, cheap_engine, engine_override, waba_id, phone_number_id, cloud_api_token)
         VALUES ($1, $2::uuid, $3, true, $4, true, true, 'active',
        ...
        [
            ...
            true, 
            engine,
            officialConfig.waba_id || null,
            officialConfig.phone_number_id || null,
            officialConfig.cloud_api_token || null
        ]
    );
    ...
    if (existingResult.rows.length === 0) {
        ...
        try {
            await query(
                'UPDATE user_configs SET message_credit = message_credit + 100 WHERE user_id::uuid = $1::uuid OR email = $2',
    ...
    try {
        await query(
            `INSERT INTO whatsapp_sessions
             ...
             VALUES ($1, $1, $2::uuid, $3, $4, $5, now(), now(), $6, '', NULL)
...
async function getWhatsAppConfigByPhoneId(phoneId) {
    try {
        const res = await query(
            'SELECT * FROM whatsapp_message_database WHERE phone_number_id = $1 LIMIT 1',
            [phoneId]
        );
        return res.rows[0] || null;
    } catch (error) {
        console.error('[DB] getWhatsAppConfigByPhoneId Error:', error.message);
        return null;
    }
}
...
async function getAllActivePages() {
    try {
        const pagesResult = await query(
            `SELECT page_id, user_id, message_credit, subscription_status, api_key, cheap_engine, permanent_credit, bonus_credit
...
async function deductWhatsAppCredit(sessionName, amount = 1) {
    try {
        ...
        if (sessionResult.rows.length === 0 || !sessionResult.rows[0].user_id) {
            console.warn(`[Credit] WhatsApp Session ${sessionName} not linked to any user.`);
            return false;
        }

        const userId = sessionResult.rows[0].user_id;
        
        // We can reuse the core logic by passing a dummy pageId or creating a base function
        // For simplicity, let's just find the first page_id for this user and use it for deductCredit
        const pageResult = await query(
            'SELECT page_id FROM page_access_token_message WHERE user_id::text = $1::text LIMIT 1',
            [String(userId)]
        );

        if (pageResult.rows.length > 0) {
            return await deductCredit(pageResult.rows[0].page_id, amount);
        } else {
            // Fallback: If user has no messenger pages, we need to deduct by user_id directly
            // (I should probably refactor deductCredit to take userId directly)
            console.warn(`[Credit] User ${userId} has no Messenger pages. Cannot deduct credit via pageId.`);
            return false;
        }
    } catch (error) {
        console.error('[DB] deductWhatsAppCredit Error:', error.message);
        return false;
    }
}

async function upsertLabelAction(pageId, labelName, aiAction = 'continue') {
    try {
        await query(
            `INSERT INTO label_actions (page_id, label_name, ai_action)
             VALUES ($1, $2, $3)
             ON CONFLICT (page_id, label_name) DO UPDATE SET
             ai_action = EXCLUDED.ai_action`,
            [pageId, labelName.toLowerCase(), aiAction]
        );
        console.log(`[DB] Upserted Label Action: ${labelName} -> ${aiAction} for ${pageId}`);
    } catch (e) {
        console.error(`[DB] Failed to upsert label action:`, e.message);
    }
}
...
// 31. Search Products (For AI) - Hybrid Search (Vector + Keyword)
async function searchProducts(userId, queryText, pageId = null) {
    try {
        ...
        // 2. Trigram Similarity for better Keyword matching
        // 3. Exact ILIKE match for specific brands/models
        // 4. Split query search with higher weight for multiple word matches
        const queryWords = cleanQuery.split(' ').filter(w => w.length > 2);
        let wordFilters = "";
        let wordScoring = "0";
        if (queryWords.length > 0) {
            // High accuracy fix: Search individual words across name, keywords and description
            wordFilters = "AND (" + queryWords.map((w, i) => `(name ILIKE $${6 + i} OR keywords::text ILIKE $${6 + i} OR description ILIKE $${6 + i})`).join(' OR ') + ")";
            // Score based on where the match is found (Name > Keywords > Description)
            wordScoring = queryWords.map((w, i) => `(CASE WHEN name ILIKE $${6 + i} THEN 0.2 WHEN keywords::text ILIKE $${6 + i} THEN 0.1 WHEN description ILIKE $${6 + i} THEN 0.05 ELSE 0 END)`).join(' + ');
        }
        ...
        let sql = `
            ...
            keyword_matches AS (
                ...
                WHERE user_id::text = $2::text 
                  AND is_active = true 
                  AND (name % $4 OR keywords::text ILIKE $5 OR name ILIKE $5 OR description ILIKE $5 ${wordFilters.replace('AND (', 'OR (')}) 
        ...
        if (pageId) {
            params.push(String(pageId));
            sql += ` WHERE ((${pageColumn}::jsonb @> jsonb_build_array($3::text)) OR platform = 'global')`;
        }

        sql += ` ORDER BY distance ASC LIMIT 15`;
        
        const finalParams = pageId ? [...params, cleanQuery, `%${cleanQuery}%`, ...queryWords.map(w => `%${w}%`)] : [...params, cleanQuery, `%${cleanQuery}%`, ...queryWords.map(w => `%${w}%`)];

        const result = await query(sql, finalParams);
        
        // Return products only if they are reasonably similar
        // distance < 0.8 is much more lenient for fuzzy/bangla/misspelled queries
        return result.rows.filter(p => p.distance < 0.8);
        
    } catch (error) {
        console.error("[DB] searchProducts Hybrid Error:", error.message);
        return [];
    }
}
