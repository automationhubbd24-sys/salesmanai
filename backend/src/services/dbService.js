...
async function deductCredit(pageId, currentCredit) {
    ...
    try {
        ...
        const userIdStr = String(pageData.user_id);
        ...
        if (isNewMonth) {
            // Reset everything for new month as per user request
            dailyUsed = 0;
            bonusCredit = 0;
            monthlyUsed = 0;
            await query(
            ...
            console.log(`[Credit] Monthly limit reset for User ${userIdStr}`);
        } else if (isNewDay) {
            dailyUsed = 0;
            await query(
            ...
            console.log(`[Credit] Daily limit reset for User ${userIdStr}`);
        }

        // --- 2. DEDUCTION LOGIC (SMART ROUTING) ---
        
        // Priority 1: Daily Limit
        if (Number(config.daily_limit || 0) > dailyUsed) {
            await query(
                'UPDATE user_configs SET daily_used = daily_used + 1, monthly_used = monthly_used + 1 WHERE user_id::text = $1',
                [userIdStr]
            );
            console.log(`[Credit] Deducted from Daily Limit for User ${userIdStr} (${dailyUsed + 1}/${config.daily_limit})`);
            return true;
        }

        // Priority 2: Bonus Credit
        if (bonusCredit > 0) {
            await query(
                'UPDATE user_configs SET bonus_credit = bonus_credit - 1, monthly_used = monthly_used + 1 WHERE user_id::text = $1',
                [userIdStr]
            );
        ...
        // Priority 3: Permanent Credit
        if (Number(config.permanent_credit || 0) > 0) {
            await query(
                'UPDATE user_configs SET permanent_credit = permanent_credit - 1 WHERE user_id::text = $1',
                [userIdStr]
            );
        ...
        // Priority 4: Legacy Message Credit (Fallback)
        if (Number(config.message_credit || 0) > 0) {
            await query(
                'UPDATE user_configs SET message_credit = message_credit - 1 WHERE user_id::text = $1',
                [userIdStr]
            );
            console.log(`[Credit] Deducted from Legacy Message Credit for User ${userIdStr}`);
            return true;
        }
...
async function deductWhatsAppCredit(sessionName, amount = 1) {
    ...
    try {
        ...
        if (Number(config.message_credit || 0) > 0) {
            ...
            console.log(`[WA Credit] Deducted from Legacy Message Credit for User ${userIdStr}`);
            return true;
        }
