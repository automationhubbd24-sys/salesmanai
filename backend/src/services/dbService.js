...
async function deductCredit(pageId, amount = 1) {
    ...
    try {
        ...
        if (Number(config.daily_limit || 0) > dailyUsed) {
            ...
            console.log(`[Credit] Deducted from Daily Limit for User ${userIdStr} (${dailyUsed + amount}/${config.daily_limit})`);
            return true;
        }
...
async function searchProducts(userId, queryText, pageId = null) {
    try {
        ...
        if (pageId) {
            params.push(String(pageId));
            sql += ` AND ((${pageColumn}::jsonb @> jsonb_build_array($3::text)) OR platform = 'global')`;
        }
