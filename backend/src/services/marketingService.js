...
class MarketingService {
    ...
    async sendBulkMessage(task) {
        ...
        try {
            ...
            const hasBonus = Number(config.bonus_credit || 0) > 0;
            const hasPermanent = Number(config.permanent_credit || 0) > 0;
            const hasDaily = Number(config.daily_limit || 0) > Number(config.daily_used || 0);
            const hasLegacy = Number(config.message_credit || 0) > 0;

            if (!config || (!hasBonus && !hasPermanent && !hasDaily && !hasLegacy)) {
