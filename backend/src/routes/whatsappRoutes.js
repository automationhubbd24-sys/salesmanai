    ...
    try {
        ...
        if (!requestedOwner || requestedOwner === userEmail) {
            const { rows } = await pgClient.query(
                'SELECT id, session_name, expires_at, plan_days, status, subscription_status, user_id, email FROM whatsapp_message_database WHERE user_id::uuid = $1::uuid OR email = $2',
                [userId, userEmail]
            );
            mySessions = rows;
        }
