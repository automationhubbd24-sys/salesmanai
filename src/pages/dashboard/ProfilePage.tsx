...
export default function ProfilePage() {
  ...
  const fetchTeamMembers = async (email: string) => {
    ...
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/teams/members${query}`, {
        headers: {
  ...
  const fetchResources = async () => {
    ...
    try {
      ...
      const [fbRes, waRes] = await Promise.all([
        secureFetch(`${BACKEND_URL}/api/messenger/pages${query}`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        secureFetch(`${BACKEND_URL}/api/whatsapp/sessions${query}`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const handleUpdateMember = async () => {
    ...
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/teams/members/${editingMemberId}${query}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleRemoveMember = async (id: string) => {
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/teams/members/${id}${query}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="bg-[#0f0f0f]/80 backdrop-blur-sm border border-white/10 h-fit shadow-[0_12px_32px_rgba(0,0,0,0.15)]">
          ...
          <CardContent className="space-y-6">
                              ...
                              availableWaSessions.map(session => (
                                ...
                                >
                                  ...
                                  <label 
                                    ...
                                    className="text-sm leading-5 text-foreground flex items-center gap-2"
                                  >
                                    {session.session_name} 
                                    <span className="text-[10px] text-muted-foreground bg-white/5 px-1.5 py-0.5 rounded">({session.status})</span>
                                    {session.api_type === 'official' ? (
                                      <span className="text-[9px] bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded font-bold uppercase tracking-tighter">Official</span>
                                    ) : (
                                      <span className="text-[9px] bg-slate-500/20 text-slate-400 px-1.5 py-0.5 rounded font-bold uppercase tracking-tighter">Unofficial</span>
                                    )}
                                  </label>
                                </div>
                              ))
            ...
            {/* Members List */}
            <div className="overflow-x-auto rounded-md border border-white/10 bg-[#0f0f0f]/60">
          ...
          <form
            onSubmit={async (e) => {
              ...
              try {
                ...
                const res = await secureFetch(`${BACKEND_URL}/api/auth/password/change`, {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
