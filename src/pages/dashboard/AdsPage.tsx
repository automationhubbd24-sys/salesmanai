...
export default function AdsPage() {
  ...
  const checkActivePages = async () => {
    try {
      ...
      const [msgRes, waRes] = await Promise.all([
        secureFetch(`${BACKEND_URL}/api/external/fb/pages?user_id=${userId}`, {
          headers: { Authorization: `Bearer ${token}` }
        }),
        secureFetch(`${BACKEND_URL}/api/external/wa/sessions?user_id=${userId}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      ]);
  ...
  const fetchAds = async () => {
    try {
      ...
      const response = await secureFetch(`${BACKEND_URL}/api/ads?${params.toString()}`, {
        headers: {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await secureFetch(`${BACKEND_URL}/api/products?${params.toString()}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleSaveAd = async () => {
    ...
    try {
      ...
      const response = await secureFetch(`${BACKEND_URL}/api/ads`, {
        method: "POST",
        headers: {
  ...
  const handleDeleteAd = async (ad_id: string, p_id: string) => {
    ...
    try {
      const token = localStorage.getItem("auth_token");
      const response = await secureFetch(`${BACKEND_URL}/api/ads?ad_id=${ad_id}&page_id=${p_id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
