...
export default function IntegrationPage() {
  ...
  const fetchSessions = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/whatsapp/sessions`, {
  ...
  useEffect(() => {
    ...
    if (qrSession && qrSession.status !== 'WORKING' && qrSession.status !== 'STOPPED') {
      const fetchQr = async () => {
          try {
              const res = await secureFetch(`${BACKEND_URL}/api/whatsapp/session/qr/${qrSession.session_name}`);
              const data = await res.json();
  ...
  const createSession = async () => {
    ...
    try {
      ...
      if (selectedEngine === "OFFICIAL") {
        ...
        payload.cloud_api_token = cloudApiToken;
      }
  ...
  const performAction = async (sessionName: string, action: 'start' | 'stop' | 'delete' | 'restart') => {
      ...
      try {
        await secureFetch(`${BACKEND_URL}/api/whatsapp/session/${action}`, {
          method: 'POST',
          headers: { 
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
