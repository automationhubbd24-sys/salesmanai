...
export default function MessengerIntegrationPage() {
    ...
    useEffect(() => {
        ...
        const initFB = () => {
            if (window.FB) {
                window.FB.init({
                    appId      : import.meta.env.VITE_FACEBOOK_APP_ID || '3741087805185945',
                    cookie     : true,
                    xfbml      : true,
                    version    : 'v19.0'
                });
                 ...
                 window.FB.api(
                    ...
                    function(response: any) {
                        ...
                        if (!response || response.error) {
                            ...
                            secureFetch(`https://graph.facebook.com/v19.0/${pageId}/subscribed_apps?access_token=${accessToken}&subscribed_fields=${fieldsStr}`, {
                                method: 'POST'
                            })
                            .then(res => res.json())
                            .then(data => {
    ...
    const unsubscribeAppFromPage = (pageId: string, accessToken: string) => {
        return new Promise((resolve) => {
            const doDirectUnsubscribe = () => {
                secureFetch(`https://graph.facebook.com/v19.0/${pageId}/subscribed_apps?access_token=${accessToken}`, {
                    method: 'DELETE'
                })
                .then(res => res.json())
                .then(data => {
            ...
            try {
                ...
                try {
                    ...
                    if (subResult?.error) {
                        ...
                        const errorCode = subResult.error.code || 'N/A';
                        
                        // User Friendly Explanations
                        let explanation = "Please check if your Facebook App has 'pages_messaging' and 'pages_manage_metadata' permissions.";
                        if (errorCode === 200) explanation = "Permissions Error: Ensure you are an Admin of this page and the app is live.";
                        if (errorCode === 100) explanation = "Invalid Parameter: Check if 'pages_manage_metadata' is approved in your Facebook App.";
                        if (errorMsg.includes('permission')) explanation = "Missing Permissions: Your App needs 'pages_messaging' approval.";

                        // Log to Backend
                        logFrontendError({
                            message: `Subscription Failed for ${page.name}: ${errorMsg} (Code: ${errorCode})`,
                            context: 'MessengerIntegrationPage:subscribeAppToPage',
                            pageName: page.name,
                            pageId: page.id,
                            fbError: subResult.error,
                            explanation: explanation
                        });

                        toast.error(`${page.name}: Connection Failed`, {
                        ...
                        try {
                            const verifySub = await secureFetch(`https://graph.facebook.com/v19.0/${page.id}/subscribed_apps?access_token=${page.access_token}`);
                ...
                const res = await secureFetch(`${BACKEND_URL}/api/messenger/pages/manual`, {
                    method: "POST",
                    headers: {
                ...
                if (exchangeResponse.ok) {
                    ...
                    if (exchangeData.access_token) {
                        ...
                        toast.error("Token Exchange Error", {
                            description: "Backend failed to return a long-lived token. Check your .env config.",
                        });
                    }
                } else {
                    ...
                    const errorMsg = errorJson.error || 'Token Exchange Failed';
                    const details = errorJson.details ? JSON.stringify(errorJson.details) : errorText;

                    logFrontendError({
                        message: `Token Exchange Failed: ${errorMsg}`,
                        context: 'MessengerIntegrationPage:handleConnectFacebook:Exchange',
                        details: details,
                        status: exchangeResponse.status
                    });

                    toast.error("Connection Failed (Exchange)", {
                        description: `Error: ${errorMsg}. Make sure your App Secret is correct.`,
                        duration: 7000,
                    });
                }
    ...
    const handleDirectConnect = async () => {
        ...
        try {
            ...
            const verifyRes = await secureFetch(`https://graph.facebook.com/v19.0/${directPageId}?fields=name&access_token=${directAccessToken}`);
    ...
    const handleRemovePage = async (page: PageData) => {
        ...
        try {
            ...
            const res = await secureFetch(`${BACKEND_URL}/api/messenger/pages/${page.page_id}`, {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${token}`,
                },
