...
export default function PlatformSelection() {
  ...
  useEffect(() => {
    ...
    async function fetchStats() {
        try {
            const res = await secureFetch(`${BACKEND_URL}/api/stats/total-sessions`);
            if (res.ok) {
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-10">
      {/* Hero Section */}
      <div className="relative overflow-hidden rounded-3xl bg-[#0b0b0b] p-8 md:p-12 border border-white/10 shadow-[0_16px_48px_rgba(0,0,0,0.5)]">
        <div className="relative z-10 max-w-2xl">
          <div className="inline-flex items-center rounded-full border border-[#00ff88]/30 bg-black/40 px-3 py-1 text-sm font-medium text-[#00ff88] backdrop-blur-sm mb-4">
            <Zap className="mr-2 h-3.5 w-3.5 text-[#00ff88] fill-[#00ff88]" />
          ...
          <div className="flex flex-wrap gap-4">
            <Button
              ...
              className="h-12 rounded-full bg-[#00ff88] text-black font-bold shadow-[0_10px_30px_rgba(0,255,136,0.25)] hover:bg-[#00f07f] transition-all hover:scale-[1.01] active:scale-95"
            >
            ...
            <Button
              ...
              size="lg"
              asChild
              className="h-12 rounded-full border-[#00ff88] text-[#00ff88] hover:bg-[#00ff88]/10"
            >
              <Link to="/dashboard/api">Developer API</Link>
            </Button>
          </div>
        </div>
        
        {/* Decorative background elements */}
        <div className="absolute right-0 top-0 h-full w-1/3 bg-gradient-to-l from-[#00ff88]/10 to-transparent opacity-50" />
        <div className="absolute -right-20 -top-20 h-96 w-96 rounded-full bg-[#00ff88]/10 blur-3xl" />
      </div>
      ...
      {/* Platforms Grid */}
      <div>
        <h2 className="text-3xl font-bold tracking-tight mb-6 text-white">Your Platforms</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {platforms.map((platform) => (
            <Card 
              key={platform.id} 
              className="group relative overflow-hidden transition-all hover:shadow-lg border-white/10 bg-[#0f0f0f]/80 backdrop-blur-sm hover:border-[#00ff88]/30 hover:shadow-[0_12px_32px_rgba(0,255,136,0.15)]"
            >
