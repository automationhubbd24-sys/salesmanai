...
import { Save, Bot, Sparkles, Key, Check, Image, Clock, Infinity as InfinityIcon, Loader2, Star, Facebook, ShieldCheck } from "lucide-react";
...
const formSchema = z.object({
  api_type: z.enum(["unofficial", "official"]),
...
export default function WhatsAppSettingsPage() {
  ...
  const handleWhatsAppEmbeddedSignup = async () => {
    ...
    try {
      ...
      // Send to backend to fetch WABA and Phone ID
      ...
      form.setValue("waba_id", data.waba_id);
      form.setValue("phone_number_id", data.phone_number_id);
      form.setValue("cloud_api_token", data.access_token);
      
      toast.success(`Connected to ${data.phone_number}! Please save your settings.`);
    } catch (error: any) {
  ...
  const handlePurchaseCredits = async () => {
    ...
    try {
      ...
      if (monthlyPlans.has(selectedPlan)) {
        ...
        const res = await secureFetch(`${BACKEND_URL}/api/auth/payments/buy-plan`, {
          method: 'POST',
        ...
        const res = await secureFetch(`${BACKEND_URL}/api/auth/payments/buy-credits`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  const fetchConfig = useCallback(async (configId: string) => {
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/whatsapp/config/${configId}`, {
      ...
      // AI Settings
      form.reset({
        api_type: dbRow.api_type || "unofficial",
        ...
        waba_id: dbRow.waba_id || "",
        phone_number_id: dbRow.phone_number_id || "",
        cloud_api_token: dbRow.cloud_api_token || "",
      });
  ...
  */
  ...
  const handleSavePrompt = async () => {
    ...
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/whatsapp/config/${dbId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
    ...
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/whatsapp/config/${dbId}`, {
        ...
        body: JSON.stringify({
          ...
          cloud_api_token: values.cloud_api_token
        }),
      });
  ...
  const handleOptimizePrompt = async () => {
    ...
    setOptimizing(true);
      try {
        const response = await secureFetch(`${BACKEND_URL}/api/ai/optimize-prompt`, {
          method: 'POST',
      ...
      <div className="grid gap-6">
        <Card className="bg-[#0f0f0f]/80 backdrop-blur-sm border border-white/10">
          ...
          <CardContent>
            {loading ? (
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                        <div className="space-y-6">
                            {/* WhatsApp Connection Type removed as it's handled during session creation */}
                        </div>

                        {form.watch("api_type") === "official" && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-xl border border-[#00ff88]/20 bg-[#00ff88]/5 mb-6">
                                <div className="md:col-span-2 flex justify-between items-center mb-2">
                                    <h4 className="text-sm font-bold text-blue-400">Quick Connect</h4>
                                    <Button 
                                        type="button"
                                        size="sm"
                                ...
                                <FormField
                                  ...
                                  render={({ field }) => (
                                    <FormItem>
                                      ...
                                      <FormControl>
                                        <Input placeholder="Enter Phone ID" {...field} />
                                      </FormControl>
                                      <FormMessage />
                                    </FormItem>
                                  )}
                                ...
                                <FormField
                                  ...
                                  render={({ field }) => (
                                    <FormItem className="md:col-span-2">
                                      ...
                                      <FormControl>
                                        <Input placeholder="EAAB..." type="password" {...field} />
                                      </FormControl>
                                      <FormDescription>
                                        Get this from your Meta Developer Dashboard.
                                      </FormDescription>
                                      <FormMessage />
                                    </FormItem>
                                  )}
            ...
            {/* Form is already wrapped at top level */}
                
                {mode === "own" ? (
                    <div className="space-y-6">
                        {/* WhatsApp Connection Type removed */}

                  {form.watch("api_type") === "official" && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-lg bg-blue-500/5 border border-blue-500/10">
                      <FormField
                        control={form.control}
                        name="waba_id"
                        render={({ field }) => (
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                Use a permanent token for best stability.
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                            ...
                            <DialogContent className="max-w-4xl bg-[#0b0b0b] border-white/10 text-white">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-6">
                                    ...
                                    <div className="space-y-4">
                                        <h4 className="text-blue-400 font-black uppercase tracking-widest text-xs flex items-center gap-2">
                                            <Key className="h-4 w-4" />
                                            WhatsApp Access
                                        </h4>
                                        <div className="space-y-3">
                                            {[
                                                { id: 'wpp_unofficial', name: 'Unofficial API', price: '500', desc: 'QR Scan / WAHA', badge: 'LIFETIME' },
                                                { id: 'wpp_official', name: 'Official Cloud API', price: '2,000', desc: 'Meta Official', badge: 'LIFETIME', color: 'text-blue-400', border: 'border-blue-500/20' }
                                            ].map((plan) => (
                                                <div 
                                                    key={plan.id}
                                                    className={`cursor-pointer relative rounded-2xl border-2 p-4 transition-all hover:border-blue-400/60 ${selectedPlan === plan.id ? 'border-blue-400 bg-blue-400/10' : 'border-white/10 bg-white/5'}`}
                                                    ...
                                                    <div className="flex justify-between items-center">
                                                        <div>
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <Badge className="bg-blue-500 text-white text-[7px] h-3 px-1">{plan.badge}</Badge>
                                                            </div>
                                                            <p className="text-[10px] text-gray-400">{plan.desc}</p>
                                                        </div>
                                                        <div className="text-right">
                                                            <div className={`text-lg font-black ${plan.color || 'text-white'}`}>৳{plan.price}</div>
                                                        </div>
                                                    </div>
                                                    {selectedPlan === plan.id && <div className="absolute -right-2 -top-2 bg-blue-400 rounded-full p-1 text-black shadow-lg"><Check className="h-3 w-3" /></div>}
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                <div className="mt-8 p-5 bg-blue-500/5 border border-blue-500/20 rounded-2xl">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1">
                                            <h4 className="text-sm font-black text-blue-400 uppercase tracking-widest mb-1">Meta Partner Compliance</h4>
                                            <p className="text-[11px] text-blue-200/70 leading-relaxed">
                                                Our platform is actively undergoing Meta Technology Provider (TP) review. 
                                                All data processing follows Meta's strict privacy standards. Your verified 
                                                Business Manager status ensures high-tier API access and stability.
                                            </p>
                                        </div>
                                    </div>
                                </div>
