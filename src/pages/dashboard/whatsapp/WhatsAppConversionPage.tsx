...
import { MessageSquare, RefreshCw, AlertCircle, Calendar as CalendarIcon, Zap, Lock, Unlock, Download, Send, Reply } from "lucide-react";
...
export default function WhatsAppConversionPage() {
  ...
  const handleOpenReply = (phoneNumber: string) => {
    setReplyTarget(phoneNumber);
    setReplyMessage("");
    setReplyDialogOpen(true);
  };

  const handleSendReply = async () => {
    if (!activeSessionName || !replyTarget || !replyMessage.trim()) return;

    setIsSendingReply(true);
    try {
        const token = localStorage.getItem("auth_token");
        const res = await secureFetch(`${BACKEND_URL}/api/whatsapp/send-message`, {
            method: "POST",
            headers: {
  ...
  const fetchContacts = async (sessionName: string) => {
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/whatsapp/contacts?${params.toString()}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const response = await secureFetch(`${BACKEND_URL}/api/whatsapp/download-conversation?${params.toString()}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleToggleLock = async (phoneNumber: string) => {
    ...
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/whatsapp/contacts/lock`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const fetchMessages = async (sessionName: string, from: Date, to: Date) => {
    ...
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/whatsapp/messages?${params.toString()}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const fetchStats = async (sessionName: string) => {
    try {
      ...
      const res = await secureFetch(`${BACKEND_URL}/api/whatsapp/stats?${params.toString()}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
                <h1 className="text-3xl font-bold tracking-tight">Conversion</h1>
                <p className="text-muted-foreground">
                Track user messages and bot automated replies for Session: 
                <span className="font-mono text-xs bg-muted px-1 py-0.5 rounded inline-flex items-center gap-1.5">
                  {activeSessionName}
                  {currentSession?.api_type === 'official' && (
                    <span className="text-[9px] bg-blue-500/20 text-blue-400 px-1 rounded font-bold uppercase tracking-tighter border border-blue-500/20">Official</span>
                  )}
                </span>
                </p>
            </div>
                                ...
                                return (
                                    <>
                                        ...
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                            onClick={() => handleOpenReply(contactId)}
                                            className="h-8 w-8 p-0"
      ...
      <Dialog open={replyDialogOpen} onOpenChange={setReplyDialogOpen}>
        <DialogContent className="sm:max-w-[425px] bg-[#0f0f0f] border border-white/10">
          <DialogHeader>
            <DialogTitle>Reply to {replyTarget?.split('@')[0]}</DialogTitle>
            <DialogDescription>
              Send a manual message to this contact.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            >
              {isSendingReply ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4 mr-2" />}
              Send Message
            </Button>
          </DialogFooter>
        </DialogContent>
