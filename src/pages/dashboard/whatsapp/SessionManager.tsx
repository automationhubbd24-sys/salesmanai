    ...
    return {
      ...
      className:
        currentSession?.name === session.name
          ...
          : "bg-slate-700/50 text-slate-300 hover:bg-slate-700/70 border-white/10",
    };
  };
    ...
    <div className="space-y-6 animate-fade-in -m-4 md:-m-6 lg:-m-6 p-4 md:p-6 lg:p-6">
      ...
      <Card id="whatsapp-connect-card" className="bg-[#121212] border-white/5">
        ...
        <CardContent className="space-y-4">
          ...
          {loading ? (
            <div className="rounded-xl border border-white/10 bg-[#0a0a0a] px-4 py-10 text-center text-slate-400">
              <Loader2 className="mx-auto h-5 w-5 animate-spin text-primary" />
              <p className="mt-3">Loading official WhatsApp connection...</p>
            </div>
          ) : officialSessions.length === 0 ? (
            <div className="rounded-xl border border-dashed border-white/10 bg-[#0a0a0a] px-4 py-12 text-center">
              <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-white/10 bg-[#121212]/50">
                <CheckCircle2 className="h-7 w-7 text-primary" />
              </div>
              <p className="mt-4 text-white font-semibold">No official WhatsApp number connected yet</p>
              <p className="mx-auto mt-1 max-w-xl text-sm text-slate-400">Click the button above to complete the official Meta flow.</p>
            </div>
          ) : (
            <div className="space-y-3 rounded-xl border border-white/10 bg-[#0a0a0a] p-4 md:p-5">
              {officialSessions.map((session) => {
                ...
                return (
                  <div
                    key={session.name}
                    className="rounded-2xl border border-white/10 bg-[#121212] p-4 transition-colors hover:border-primary/30"
                  >
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                      <div className="min-w-0 space-y-2">
                        ...
                        <div className="flex flex-col gap-1 text-xs text-slate-400 md:flex-row md:flex-wrap md:gap-4">
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row">
                        <Button variant="outline" className="min-w-[140px] border-white/10 hover:bg-white/5" onClick={() => handleReconnectHelp(session)}>
                          <RotateCcw className="mr-2 h-4 w-4" />
                          Reconnect
                        </Button>
                        <Button className="min-w-[140px] bg-primary text-black hover:bg-primary/90" onClick={() => handleManage(session)}>
