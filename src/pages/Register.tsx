...
const Register = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await secureFetch(`${BACKEND_URL}/api/auth/register`, {
        method: "POST",
        headers: {
      ...
      const otpRes = await secureFetch(`${BACKEND_URL}/api/auth/request-otp`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await secureFetch(`${BACKEND_URL}/api/auth/verify-otp`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <div className="flex flex-1 flex-col justify-center px-4 py-8 sm:px-6 lg:flex-none lg:px-16 xl:px-20">
        <div className="mx-auto w-full max-w-md">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Create Free Account</h1>
            <p className="text-gray-400">Join the elite AI automation empire today.</p>
          </div>
          ...
          <p className="mt-6 text-center text-xs text-gray-400">
            ...
            <Link to="/terms-of-service" className="text-[#00ff88] hover:underline">{t("Terms of Service", "পরিষেবার শর্তাবলী")}</Link>
            {" "}{t("and", "এবং")}{" "}
            <Link to="/privacy-policy" className="text-[#00ff88] hover:underline">{t("Privacy Policy", "গোপনীয়তা নীতি")}</Link>
          </p>
