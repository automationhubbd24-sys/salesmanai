            ...
            <SelectItem key={session.name} value={session.name}>
              <div className="flex items-center justify-between w-full gap-2">
                ...
                {session.api_type === 'official' ? (
                  <span className="text-[8px] bg-blue-500/20 text-blue-400 px-1 rounded font-bold uppercase shrink-0">Official</span>
                ) : (
                  <span className="text-[8px] bg-slate-500/20 text-slate-400 px-1 rounded font-bold uppercase shrink-0">Unofficial</span>
                )}
              </div>
            </SelectItem>
          ))}
