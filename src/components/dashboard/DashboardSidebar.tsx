...
export function DashboardSidebar({ isMobile, onLinkClick }: { isMobile?: boolean; onLinkClick?: () => void }) {
  ...
  return (
    ...
    >
      ...
      {isMobile && onLinkClick && (
        <div className="p-4 border-b border-white/5 bg-black/20">
          <Button
            ...
            variant="ghost"
            onClick={onLinkClick}
            className="w-full h-12 rounded-2xl border border-[#00ff88]/20 text-[#00ff88] text-sm font-black hover:bg-[#00ff88]/10 flex items-center justify-center gap-2"
          >
      ...
      <nav className="flex-1 overflow-y-auto p-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden space-y-4">
        ...
        <ul className="space-y-1.5 rounded-2xl p-2.5 bg-[#0f0f0f]/80 backdrop-blur-sm border border-white/10 shadow-[0_12px_32px_rgba(0,0,0,0.15)]">
          {/* Switch Platform (Highlighted) */}
          {menu.switchItem && (
            <li key={menu.switchItem.path} className="mb-3">
              <Link
                to={menu.switchItem.path}
                onClick={onLinkClick}
                className={cn(
                  "flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-200 text-sidebar-foreground border border-[#00ff88]/20 bg-[#00ff88]/5 hover:bg-[#00ff88]/10 hover:text-[#00ff88] hover:border-[#00ff88]/40 shadow-[0_4px_12px_rgba(0,255,136,0.05)]",
                  isCollapsed && "justify-center px-2"
                )}
              >
                <menu.switchItem.icon size={20} className="shrink-0 text-[#00ff88]" />
                {!isCollapsed && <span className="text-[13px] font-black uppercase tracking-wider">{menu.switchItem.title}</span>}
              </Link>
            </li>
          )}
