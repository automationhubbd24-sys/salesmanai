...
const Footer = () => {
  ...
  return (
    <footer id="footer" className="border-t border-border bg-background relative overflow-hidden pt-12 pb-12">
      {/* Background Glows (Gaming Vibe) */}
      <div className="absolute top-0 right-[-10%] w-[40%] h-[60%] bg-indigo-600/5 blur-[150px] rounded-full pointer-events-none animate-pulse dark:bg-indigo-900/10" />
      <div className="absolute bottom-0 left-[-10%] w-[40%] h-[60%] bg-purple-600/5 blur-[150px] rounded-full pointer-events-none animate-pulse dark:bg-purple-900/10" style={{ animationDelay: '2s' }} />
      
      {/* Animated Grid Overlay */}
      <div className="absolute inset-0 opacity-[0.02] pointer-events-none" style={{ backgroundImage: 'radial-gradient(var(--border) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
      ...
      <div className="container relative z-10 mx-auto px-4">
        ...
        <div className="grid gap-16 md:grid-cols-2 lg:grid-cols-6">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-lg text-muted-foreground font-medium leading-relaxed max-w-sm mb-10">
            ...
            <div className="flex gap-4">
              {[
                ...
                <motion.a
                  ...
                  className={`w-12 h-12 rounded-2xl ${bg} ${hoverBg} text-white flex items-center justify-center transition-all shadow-[0_10px_20px_rgba(0,0,0,0.1)] dark:shadow-[0_10px_20px_rgba(0,0,0,0.35)]`}
                >
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4 className="mb-8 font-black text-foreground uppercase tracking-[0.2em] text-[10px]">{title}</h4>
              <ul className="space-y-5">
                {links.map((link) => (
                  <li key={link.name}>
                    <Link
                      to={link.href}
                      className="text-[13px] text-muted-foreground font-bold transition-all hover:text-indigo-500 hover:translate-x-1 inline-block"
                    >
                      {link.name}
                    </Link>
                  </li>
        ...
        {/* Bottom Footer */}
        <div className="mt-32 pt-10 border-t border-border flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex items-center gap-8">
            <p className="text-xs text-muted-foreground font-bold">© {new Date().getFullYear()} SalesmanChatbot.</p>
            <div className="flex gap-6">
              <Link to="/privacy-policy" className="text-[10px] text-muted-foreground font-black uppercase tracking-widest hover:text-foreground transition-colors">{t("Privacy", "প্রাইভেসি")}</Link>
              <Link to="/terms-of-service" className="text-[10px] text-muted-foreground font-black uppercase tracking-widest hover:text-foreground transition-colors">{t("Terms", "শর্তাবলী")}</Link>
            </div>
          </div>
