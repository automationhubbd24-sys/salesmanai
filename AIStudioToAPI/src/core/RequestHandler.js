...
const AI_STUDIO_GENERATION_FALLBACK_MODELS = [
    "gemini-3.5-flash",
    "gemini-flash-latest",
...
class RequestHandler {
    ...
    _getTrackedModelName(proxyRequest) {
        return this._normalizeModelName(proxyRequest?.tracking_model || this._extractModelFromPath(proxyRequest?.path));
    }

    _getAiStudioFallbackChain(modelName) {
        const normalizedModel = this._normalizeModelName(modelName);
        if (!normalizedModel) return [];

        // LIMIT FALLBACK SCOPE: Do not apply fallback for preview models
        if (normalizedModel.includes('preview')) {
            console.log(`[AI Studio] Skipping fallback for preview model: ${normalizedModel}`);
            return [];
        }

        const modelIndex = AI_STUDIO_GENERATION_FALLBACK_MODELS.indexOf(normalizedModel);
        if (modelIndex === -1) return [];

        return AI_STUDIO_GENERATION_FALLBACK_MODELS.slice(modelIndex);
    }

    _replaceModelReferences(node, currentModel, nextModel) {
        if (!node || !currentModel || !nextModel) return;

        if (Array.isArray(node)) {
            for (const item of node) {
                this._replaceModelReferences(item, currentModel, nextModel);
            }
            return;
        }
        ...
        for (const [key, value] of Object.entries(node)) {
            if (key === "model" && typeof value === "string") {
                const normalizedValue = this._normalizeModelName(value);
                if (normalizedValue === currentModel) {
                    node[key] = value.startsWith("models/") ? `models/${nextModel}` : nextModel;
                }
                continue;
            }

            this._replaceModelReferences(value, currentModel, nextModel);
        }
    }

    _shouldFallbackToNextModel(proxyRequest, errorPayload) {
        if (!proxyRequest?.is_generative) return false;
        if (this._categorizeRequest(proxyRequest?.path, "request") !== "generation") return false;

        const currentModel = this._getTrackedModelName(proxyRequest);
        const fallbackChain = this._getAiStudioFallbackChain(currentModel);
        if (fallbackChain.length <= 1) return false;
    ...
    _switchProxyRequestToNextModel(proxyRequest, errorPayload) {
        if (!this._shouldFallbackToNextModel(proxyRequest, errorPayload)) {
            return false;
        }
        ...
        const pathModel = this._extractModelFromPath(proxyRequest.path);
        if (pathModel) {
            proxyRequest.path = proxyRequest.path.replace(`/models/${pathModel}`, `/models/${nextModel}`);
        }

        proxyRequest.tracking_model = nextModel;

        if (typeof proxyRequest.body === "string" && proxyRequest.body) {
            try {
                const parsedBody = JSON.parse(proxyRequest.body);
                this._replaceModelReferences(parsedBody, currentModel, nextModel);
                proxyRequest.body = JSON.stringify(parsedBody);
            } catch (error) {
                this.logger.debug(
                    `[Model Fallback] Failed to rewrite request body for fallback from ${currentModel} to ${nextModel}: ${error.message}`
                );
            }
        }
        ...
        this.logger.warn(
            `[Model Fallback] Request #${proxyRequest.request_id} switching model ${currentModel} -> ${nextModel} after status ${errorPayload?.status || "unknown"} (${errorPayload?.message || "no message"})`
        );
    ...
    async _prepareImmediateStatusRetry(errorDetails, requestId, tracker, sourceAuthIndex) {
        ...
        this.logger.debug(
            `[Request] Immediate retry decision for request #${requestId}: status=${errorDetails?.status}, sourceAuthIndex=${sourceAuthIndex}, currentAuthIndex=${currentAuthIndex}`
        );
        ...
        if (hasSourceAuth && hasCurrentAuth && sourceAuthIndex !== currentAuthIndex) {
            ...
            if (this.authSwitcher.isAuthTemporarilyUnavailable(retryAuthIndex)) {
                this.logger.warn(
                    `[Request] Current account #${retryAuthIndex} is cooling down after recent failures; ` +
                        `performing a fresh switch for request #${requestId}.`
                );
                return this._performImmediateSwitchRetry(errorDetails, requestId, tracker);
            }
            ...
            try {
                ...
                if (wantsStream) {
                    if (proxyRequest.streaming_mode === "fake") {
                        await this._handlePseudoStreamResponse(proxyRequest, messageQueue, req, res, authIndex);
                    } else {
                        await this._handleRealStreamResponse(proxyRequest, messageQueue, req, res, authIndex);
                    }
                } else {
                    proxyRequest.streaming_mode = "fake";
                    await this._handleNonStreamResponse(proxyRequest, messageQueue, req, res, authIndex);
                }
            } catch (error) {
        ...
        try {
            ...
            try {
                ...
                if (useRealStream) {
                    ...
                    let currentQueueAuthIndex = authIndex;
                    let initialMessage;
                    ...
                    await this._streamOpenAIResponse(currentQueue, res, proxyRequest.tracking_model || model, requestId);
                } else {
                    ...
                    try {
                        const result = await this._executeRequestWithRetries(proxyRequest, messageQueue, authIndex);
                        ...
                        if (isOpenAIStream) {
                            ...
                            try {
                                ...
                                const translatedChunk = this.formatConverter.translateGoogleToOpenAIStream(
                                    fullBody,
                                    proxyRequest.tracking_model || model,
                                    streamState
                                );
                                if (this._isResponseWritable(res)) {
                                    try {
                            ...
                            // Non-stream
                            await this._sendOpenAINonStreamResponse(
                                activeQueue,
                                res,
                                proxyRequest.tracking_model || model,
                                requestId
        ...
        try {
            ...
            try {
                ...
                if (useRealStream) {
                    ...
                    await this._streamOpenAIResponseAPIResponse(currentQueue, res, proxyRequest.tracking_model || model, {
                        requestId,
                        responseDefaults,
                    });
                } else {
        ...
        try {
            ...
            try {
                ...
                if (useRealStream) {
                    ...
                    await this._streamClaudeResponse(currentQueue, res, proxyRequest.tracking_model || model, requestId);
                } else {
                    ...
                    try {
                        ...
                        if (isClaudeStream) {
                            ...
                            // Non-stream
                            await this._sendClaudeNonStreamResponse(
                                activeQueue,
                                res,
                                proxyRequest.tracking_model || model,
                                requestId
    ...
    async _handlePseudoStreamResponse(proxyRequest, messageQueue, req, res, authIndex = this.currentAuthIndex) {
    ...
    async _handleRealStreamResponse(proxyRequest, messageQueue, req, res, authIndex = this.currentAuthIndex) {
        let currentQueue = messageQueue;
        let currentQueueAuthIndex = authIndex;
        let headerMessage;
        let skipFinalFailureSwitch = false;
    ...
    async _handleNonStreamResponse(proxyRequest, messageQueue, req, res, authIndex = this.currentAuthIndex) {
        try {
            const result = await this._executeRequestWithRetries(proxyRequest, messageQueue, authIndex);
    ...
    async _executeRequestWithRetries(proxyRequest, messageQueue, authIndex = this.currentAuthIndex) {
        let lastError = null;
        let currentQueue = messageQueue;
        ...
        while (retryAttempt <= this.maxRetries) {
            ...
            try {
                ...
                if (
                    ...
                    !Number.isInteger(errorPayload?.authIndex)
                ) {
                    errorPayload.authIndex = currentQueueAuthIndex;
                }
                ...
                if (retryAttempt >= this.maxRetries) {
                    const switchedModel = this._switchProxyRequestToNextModel(proxyRequest, errorPayload);
                    if (switchedModel) {
                        try {
                            currentQueue.close("model_fallback_creating_new_queue");
                        } catch (e) {
                            this.logger.debug(`[Request] Failed to close old queue before model fallback: ${e.message}`);
                        }
                        ...
                        if (
                            !(await this._waitForSystemAndConnectionIfBusy(null, {
                                connectionMessage: "Service temporarily unavailable: Connection not ready before model fallback.",
                            }))
                        ) {
                            lastError = {
                                message: `WebSocket connection not ready before model fallback on account #${this.currentAuthIndex}.`,
                                status: 503,
                            };
                            break;
                        }
    ...
    _buildProxyRequest(req, requestId) {
        ...
        return {
            ...
            tracking_model: this._extractModelFromPath(cleanPath),
        };
    }
    ...
    _forwardRequest(proxyRequest, authIndex = this.currentAuthIndex) {
        ...
        if (connection) {
            ...
            throw new Error(
                `Unable to forward request: No WebSocket connection found for authIndex=${authIndex}`
            );
        }
    }
