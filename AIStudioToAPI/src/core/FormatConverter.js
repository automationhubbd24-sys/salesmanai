        ...
        for (let msgIndex = 0; msgIndex < conversationMessages.length; msgIndex++) {
            ...
            if (typeof message.content === "string" && message.content.length > 0) {
                ...
                for (const part of message.content) {
                    ...
                    // Support OpenAI input_audio format (Base64)
                    ...
                    if (audioData) {
                        ...
                        this.logger.info(`[Adapter] Added input_audio part (format: ${format})`);
                    }
                } else if (part.type === "audio_url" && part.audio_url) {
                    // Support audio_url format (URL)
                    const audioUrl = typeof part.audio_url === "string" ? part.audio_url : part.audio_url.url;
                    if (audioUrl) {
                        try {
                            this.logger.info(`[Adapter] Downloading audio from URL: ${audioUrl}`);
                            ...
                            let mimeType = response.headers["content-type"];
                            if (!mimeType || mimeType === "application/octet-stream") {
                                mimeType = mime.lookup(audioUrl) || "audio/mpeg"; // Fallback
                            }
                            googleParts.push({
                                inlineData: {
                                    data: base64Data,
                            ...
                            this.logger.info(`[Adapter] Successfully downloaded and converted audio to base64.`);
                        } catch (error) {
                            this.logger.error(
                                `[Adapter] Failed to download or process audio from URL: ${audioUrl}`,
                                error
                            );
                            googleParts.push({ text: `[System Note: Failed to load audio from ${audioUrl}]` });
                        }
                    }
                }
            }
