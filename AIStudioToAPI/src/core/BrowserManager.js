...
class BrowserManager {
    constructor(logger, config, authSource) {
        ...
        this.targetUrl = "https://ai.studio/apps/d95c5b9c-cf68-4419-90ef-6546f471f731";
     ...
     */
    _startHealthMonitorForContext(authIndex) {
        // Get context data
        const contextData = this.contexts.get(authIndex);
        if (!contextData) {
        ...
        contextData.healthMonitorInterval = setInterval(async () => {
            try {
                ...
                try {
                    ...
                    // 3. Auto-Save Auth: Every ~24 hours (21600 ticks * 4s = 86400s) - only for active context
                    if (tickCount % 21600 === 0 && this._currentAuthIndex === authIndex) {
                        try {
                            this.logger.info(
    ...
    _startHealthMonitor() {
        // Start health monitor for ALL preloaded contexts
        for (const authIndex of this.contexts.keys()) {
            this._startHealthMonitorForContext(authIndex);
        }
    }
    ...
    async _startBackgroundWakeup() {
        ...
        // Initial buffer - wait before starting the main loop to let pages stabilize
        await new Promise(r => setTimeout(r, 1500));

        // Verify at least one context exists after the initial delay
        try {
            if (this.contexts.size === 0) {
                this.backgroundWakeupRunning = false;
                this.logger.info(
                    "[Browser] BackgroundWakeup stopped: no contexts available during startup delay."
                );
                return;
            }
        } catch (error) {
            this.backgroundWakeupRunning = false;
            this.logger.warn(`[Browser] BackgroundWakeup stopped: error checking context status: ${error.message}`);
            return;
        }

        this.logger.info("[Browser] 🛡️ Background Wakeup Service (Rocket Handler) started...");

        // Main loop: check ALL preloaded contexts for Launch buttons!
        while (this.contexts.size > 0) {
            try {
                ...
                for (const [authIndex, contextData] of this.contexts.entries()) {
                    ...
                    if (!currentPage || currentPage.isClosed()) continue;
                    ...
                    try {
                        ...
                        if (targetInfo.found) {
                            foundButton = true;
                            this.logger.info(`[Context#${authIndex}] 🎯 Found Rocket/Launch button [${targetInfo.tagName}], engaging...`);
                            ...
                            this.logger.info(`[Context#${authIndex}] 🖱️ Physical click executed. Verifying...`);
                            await new Promise(r => setTimeout(r, 1500));
                            ...
                            if (isStillThere) {
                                this.logger.warn(`[Context#${authIndex}] ⚠️ Physical click ineffective, attempting JS force click...`);
                        ...
                        this.logger.debug(`[Context#${authIndex}] Error in background wakeup: ${pageErr.message}`);
                    }
                }

                // 4. Sleep logic
                if (foundButton) {
                    // Found and clicked a button, reset noButtonCount and sleep a little
                    this.noButtonCount = 0;
                ...
                this.logger.warn(`[BackgroundWakeup] Global error: ${globalErr.message}`);
                await new Promise(r => setTimeout(r, 1000));
            }
        }
    ...
    async _initializeContext(authIndex, isBackgroundTask = false) {
        ...
        try {
            ...
            if (!this.abortedContexts.has(authIndex) && !(isBackgroundTask && this._backgroundPreloadAbort)) {
                ...
                // Start health monitor for this new context!
                this._startHealthMonitorForContext(authIndex);
            } else {
                throw new ContextAbortedError(authIndex, "marked for deletion");
            }
